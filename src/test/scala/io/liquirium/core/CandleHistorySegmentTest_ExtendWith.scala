...
class CandleHistorySegmentTest_ExtendWith extends CandleHistorySegmentTest {
  ...
  test("it throws an exception when the starts are not aligned with regard to the candle length") {
    val s1 = segment(c5(sec(10), 1))
  ...
  test("it throws an exception when the candle lengths are not equal") {
    val s1 = segment(c5(sec(10), 1))
