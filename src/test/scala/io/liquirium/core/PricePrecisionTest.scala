...
class PricePrecisionTest extends BasicTest {

  def roundToDigitsAfterSeparator(n: Int)(p: BigDecimal): BigDecimal = PricePrecision.digitsAfterSeparator(n).apply(p)

  def roundToMultiple(step: BigDecimal)(p: BigDecimal): BigDecimal = PricePrecision.multipleOf(step).apply(p)

  def roundToSignificantDigits(n: Int, decimalsAfterPoint: Option[Int] = None)(p: BigDecimal): BigDecimal =
    PricePrecision.significantDigits(n, maxDecimalsAfterPoint = decimalsAfterPoint).apply(p)

  // infinite

  test("the infinite precision does not change the prices") {
    PricePrecision.Infinite(dec("0.123456789")) shouldEqual dec("0.123456789")
  }

  test("next higher and lower price are the given price for the infinite precision") {
    PricePrecision.Infinite.nextHigher(dec("1.1")) shouldEqual dec("1.1")
    PricePrecision.Infinite.nextLower(dec("1.1")) shouldEqual dec("1.1")
  }
  ...
  test("next higher and lower prices can be obtained for decimals-after-point precision") {
    val prec = PricePrecision.digitsAfterSeparator(2)
  ...
  test("next higher and lower prices can be obtained for multiple-of precision") {
    val prec = PricePrecision.multipleOf(0.5)
  ...
  test("the inverse multiple-of precision changes as if the inverse price was rounded to the next matching price") {
    def prec(step: BigDecimal) = PricePrecision.inverseMultipleOf(step)
  ...
  test("next higher and lower prices can be obtained for inverse multiple-of precision") {
    val prec = PricePrecision.inverseMultipleOf(0.5)
  ...
  test("next higher and lower prices can be obtained for significant digits precision") {
    val twoDigitPrec = PricePrecision.significantDigits(2)
  ...
  test("removing extra digits after moving to next higher number can in some cases lead to an even higher number") {
    def precision(sig: Int, maxDec: Int) = PricePrecision.significantDigits(sig, Some(maxDec))
