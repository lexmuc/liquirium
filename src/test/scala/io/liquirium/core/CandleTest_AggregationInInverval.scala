...
import io.liquirium.core.helper.CandleHelpers.{candle, emptyCandle}
import io.liquirium.core.helper.CoreHelpers._
import java.time.{Duration, Instant}

class CandleTest_AggregationInInterval extends BasicTest {

  private def aggregate(start: Instant = sec(0), length: Duration = secs(1))(cc: Candle*): Candle =
    Candle.aggregateInInterval(start, length, cc)

  test("the aggregated candle has the given start and length") {
    val c = aggregate(sec(12), secs(34))(
      candle(start = sec(15), length = secs(2))
    )
    c.startTime shouldEqual sec(12)
    c.length shouldEqual secs(34)
  }
  ...
  test("if all candles are empty the result candle is empty as well") {
    aggregate(start = sec(10), length = secs(5))(
      emptyCandle(start = sec(5), length = secs(5)),
      emptyCandle(start = sec(20), length = secs(5))
    ) shouldEqual emptyCandle(start = sec(10), length = secs(5))
  }
