...
class BinanceModelConverterTest_ExtractTrade extends BinanceModelConverterTest {
  ...
  test("trade id is assigned as a string id and order id as a string") {
    extractFromTradeReport(executionReport(tradeId = Some(333))).id shouldEqual StringTradeId("333")
