...
class BinanceRestApiTest_Candles extends BinanceRestApiTest {
  ...
  private def getCandles(symbol: String = "tABCDDEF",
                         interval: BinanceCandleLength = BinanceCandleLength.oneDay,
                         limit: Option[Int] = None,
                         from: Option[Instant] = None,
                         until: Option[Instant] = None) =
    api.sendRequest(CandlesRequest(symbol = symbol, interval, limit, from = from, until = until))
  ...
  test("requesting candles yields a get request with the proper url") {
    getCandles("IOTBTC", BinanceCandleLength("XYZ", secs(1)))
  ...
  test("it parses the candles with the json converter and returns them as a sequence") {
    val f = getCandles()
