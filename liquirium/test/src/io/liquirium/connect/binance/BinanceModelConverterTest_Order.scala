...
class BinanceModelConverterTest_Order extends BinanceModelConverterTest {
  ...
  test("the side determines the sign of the original quantity") {
    convert(bo(originalQuantity = dec(7), side = Side.Buy)).fullQuantity shouldEqual dec(7)
    convert(bo(originalQuantity = dec(8), side = Side.Sell)).fullQuantity shouldEqual dec(-8)
  }

  test("executed and original quantity determine the remaining quantity (side taken into account for sign)") {
    convert(bo(originalQuantity = dec(7), executedQuantity = dec(3), side = Side.Buy)).openQuantity shouldEqual dec(4)
    convert(bo(originalQuantity = dec(8), executedQuantity = dec(5), side = Side.Sell)).openQuantity shouldEqual dec(-3)
  }
